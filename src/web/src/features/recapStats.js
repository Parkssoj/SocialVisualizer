/**
Recap 페이지 통계 카드들이 공유하는 순수 헬퍼 — 통계 API 호출, 발신자 랭킹 정렬, 아바타 이니셜/색상,
워드클라우드 폰트 크기, 동기화 시간/날짜 포맷. DOM을 직접 건드리지 않는 순수 함수만 모아서 React
컴포넌트에서 재사용한다.

Shared pure helpers for the Recap page's stat cards — stat API calls, sender ranking, avatar
initials/colors, word-cloud font sizing, sync time/date formatting. No direct DOM access, reused
across the Recap React components.
 */
...
export const WC_COLORS = [
  '#353535', '#555555', '#757575', '#e63946', '#457b9d',
  '#e07b39', '#7b2d8b', '#1d6fa0', '#b5451b', '#565656',
];

export const AF_COLORS = ['#ec4899', '#a78bfa', '#f97316', '#06b6d4', '#808080', '#f59e0b', '#ef4444'];
...
export function rankKeywords(data) {
  return (data.keywords || []).slice().sort((a, b) => b.count - a.count).slice(0, 10);
}
...
export function rankAffinity(data) {
  ...
  const segments = sorted.map((item, i) => {
    ...
    const seg = { item, score, color: AF_COLORS[i % AF_COLORS.length], length, dashOffset: -offset };
    offset += length;
    return seg;
  });
