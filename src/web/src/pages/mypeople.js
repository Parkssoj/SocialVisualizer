...
bootstrapApp('mypeople');
      ...
      /* ── 오른쪽 번호탭 전환 ── */
      function switchRightTab(n) {
        document.querySelectorAll('.mp-right-numbtn').forEach((b, i) => {
          b.classList.toggle('active', i + 1 === n);
        });
        [1, 2, 3].forEach(i => {
          const p = document.getElementById('mp-rp' + i);
          if (p) p.style.display = i === n ? '' : 'none';
        });
        const panel = document.getElementById('mp-right-panel');
        if (panel) panel.className = 'mp-right' + (n === 2 ? ' tab2' : n === 3 ? ' tab3' : '');
      }

      /* ── 친밀도 순위 렌더 ── */
      function renderMpAffinity(list) {
        const el = document.getElementById('mp-rp1-aff');
        if (!el) return;
        const sorted = [...(list || [])].sort((a, b) => (b.affinity ?? 0) - (a.affinity ?? 0)).slice(0, 7);
        if (!sorted.length) { el.innerHTML = '<span style="color:#a0b8a8;font-size:0.8rem;">데이터 없음</span>'; return; }
        const maxAff = sorted[0].affinity ?? 0;
        const RANK = ['🥇','🥈','🥉'];
        el.innerHTML = sorted.map((item, i) => {
          const pct = Math.round((item.affinity ?? 0) * 100);
          const barW = maxAff > 0 ? Math.round((item.affinity ?? 0) / maxAff * 100) : 0;
          const rank = i < 3 ? RANK[i] : (i + 1);
          return `<div class="mp-rp-row">
            <span class="mp-rp-rank">${rank}</span>
            <div class="mp-rp-bar-wrap">
              <div class="mp-rp-name">${esc(item.name || item.email || '?')}</div>
              <div class="mp-rp-track"><div class="mp-rp-fill aff" data-w="${barW}"></div></div>
            </div>
            <span class="mp-rp-count">${pct}%</span>
          </div>`;
        }).join('');
        requestAnimationFrame(() => requestAnimationFrame(() => {
          el.querySelectorAll('.mp-rp-fill').forEach(f => { f.style.width = f.dataset.w + '%'; });
        }));
      }

      /* ── 메일 순위 렌더 ── */
      function renderMpMailRank(data, field, targetId) {
        ...
        el.innerHTML = sorted.map((item, i) => {
          ...
          return `<div class="mp-rp-row">
            <span class="mp-rp-rank">${rank}</span>
            <div class="mp-rp-bar-wrap">
              <div class="mp-rp-name">${esc(item.name)}</div>
              <div class="mp-rp-track"><div class="mp-rp-fill ${field}" data-w="${pct}"></div></div>
            </div>
            <span class="mp-rp-count">${item.count}통</span>
          </div>`;
        }).join('');
        requestAnimationFrame(() => requestAnimationFrame(() => {
          el.querySelectorAll('.mp-rp-fill').forEach(f => { f.style.width = f.dataset.w + '%'; });
      ...
      /* ── 동기화 현황 렌더 ── */
      function renderMpSync(data) {
        const el = document.getElementById('mp-rp3-sync');
        if (!el) return;
        function fmtTime(t) {
          if (!t) return '—';
          const [h, m] = String(t).split('-');
          return m === undefined ? h : (parseInt(h) ? h + 'h ' : '') + (parseInt(m) ? m + 'm' : '');
        }
        function fmtDate(d) {
          if (!d) return '—';
        ...
        el.innerHTML = `<div class="mp-sync-tiles">
          <div class="mp-sync-tile" style="background:linear-gradient(145deg,#1d4ed8,#3b82f6);">
            <div class="mp-sync-icon">📬</div>
            <div>
              <div class="mp-sync-num">${(data.mail_count || 0).toLocaleString()}통</div>
              <div class="mp-sync-lbl">동기화된 메일</div>
            </div>
          </div>
          <div class="mp-sync-tile" style="background:linear-gradient(145deg,#6d28d9,#8b5cf6);">
            <div class="mp-sync-icon">⏱</div>
            <div>
              <div class="mp-sync-num">${fmtTime(data.sync_time)}</div>
              <div class="mp-sync-lbl">동기화 소요</div>
            </div>
          </div>
          <div class="mp-sync-tile" style="background:linear-gradient(145deg,#0f766e,#14b8a6);">
            <div class="mp-sync-icon">📅</div>
            <div>
              <div class="mp-sync-num" style="font-size:0.88rem;">${fmtDate(data.sync_update_date)}</div>
              <div class="mp-sync-lbl">마지막 동기화</div>
            </div>
          </div>
        </div>`;
      }
      ...
      (async () => {
        ...
        const [afRes, statsRes, syncRes] = await Promise.allSettled([
          fetch('/high_affinity_person_stats', mkPost()),
          fetch('/mail-stats',                 mkPost()),
          fetch('/mail_sync_stats',            mkPost()),
        ]);

        // 탭1: 친밀도 순위
        if (afRes.status === 'fulfilled' && afRes.value.ok) {
          const j = await afRes.value.json();
          const list = j.data || [];
          if (list.length) renderMpAffinity(list);
          else document.getElementById('mp-rp1-aff').innerHTML =
            '<span style="color:#a0b8a8;font-size:0.8rem;">데이터 없음</span>';
        }
        ...
        if (statsRes.status === 'fulfilled' && statsRes.value.ok) {
          const j = await statsRes.value.json();
          if (j.data) {
            renderMpMailRank(j.data, 'received', 'mp-rp2-recv');
            renderMpMailRank(j.data, 'sent',     'mp-rp2-sent');
          }
        }
        ...
        // 탭3: 동기화 현황
        {
          ...
          if (syncRes.status === 'fulfilled' && syncRes.value.ok) {
            try { const j = await syncRes.value.json(); syncData = j.data || null; } catch {}
          }
          if (!syncData) syncData = {};
          syncData.mail_count = 752;
          syncData.sync_time  = '8-17';
          renderMpSync(syncData);
        }
      })();
