...
/**
"소셜 데이터 수집" 페이지(imap-collect.html) 전체를 감싸는 최상위 React 컴포넌트 — 헤더와 메일/메신저
탭 폼, job 로그 패널 마크업을 마운트한다. IMAP 로그인, 폴더 조회, 수집 시작, SSE로 진행상황 추적,
카카오톡 대화 업로드 같은 실제 동작은 imapCollectEngine.js(기존 로직을 거의 그대로 포팅한 모듈)가
담당하며, 이 컴포넌트는 마운트 직후 딱 한 번 initImapCollectPage()를 호출해 그 엔진을 이 DOM에
연결해준다(My Time/My People과 동일한 패턴 — 구조는 React가 그리고, 내부 동작은 useEffect 안에서
기존 엔진이 담당).

참고: 원본 HTML에는 사이드바(#app-sidebar)와 푸터(#app-footer)가 없다 — 이 페이지는 헤더만 쓴다.
그대로 유지했다.

Top-level React component wrapping the entire "social data collection" page (imap-collect.html) —
mounts the header and the mail/messenger tab forms + job log panel markup. Actual behavior (IMAP
login, folder listing, starting a collection, SSE progress tracking, KakaoTalk chat upload) is
owned by imapCollectEngine.js (a module that ports the original logic nearly verbatim); this
component just calls initImapCollectPage() once right after mount to wire that engine up to this
DOM (same pattern as My Time/My People — structure drawn by React, behavior owned by the existing
engine inside a useEffect).

Note: the original HTML has no sidebar (#app-sidebar) or footer (#app-footer) — this page only
uses the header. Kept as-is.
 */
          ...
          <div className="gw-split-grid-wrapper">
            ...
            <div className="gw-split-header-left">
              ...
              <div className="gw-tabs" role="tablist">
                <button type="button" className="gw-tab-btn active" id="tab-btn-mail" role="tab" aria-selected="true">
                  <i className="bi bi-envelope"></i> 메일
                </button>
                <button type="button" className="gw-tab-btn" id="tab-btn-message" role="tab" aria-selected="false">
                ...
                <div className="gw-collect-cards-column">
                  ...
                  <div className="gw-card">
                    ...
                    <div className="gw-presets">
                      <div className="gw-preset-chip active" data-host="imap.gmail.com" data-port="993" data-domain="gmail.com">
                        <i className="bi bi-google"></i> Gmail
                      </div>
                      <div className="gw-preset-chip" data-host="imap.naver.com" data-port="993" data-domain="naver.com">
                        <svg className="gw-brand-icon" viewBox="0 0 20 20" width="14" height="14" aria-hidden="true">
                          <rect width="20" height="20" rx="5" fill="#03C75A" />
                          <path d="M12.6 11.14 7.28 3.6H3.6v12.8h4.12V9.2l5.32 7.2H16.4V3.6h-3.8z" fill="#fff" />
                        </svg>
                        NAVER
                      </div>
                      <div className="gw-preset-chip" data-host="imap.mail.me.com" data-port="993" data-domain="icloud.com">
                    ...
                    <div className="gw-form-grid gw-uniform-grid" id="imap-server-grid">
                      <div className="gw-form-group">
                        ...
                        <input id="imap-user" className="gw-input" type="text" placeholder="you@example.com" defaultValue="@gmail.com" />
                      </div>
                      <div className="gw-form-group">
                        <label className="gw-label">앱 비밀번호</label>
                        <input id="imap-pass" className="gw-input" type="password" placeholder="앱 전용 비밀번호" />
                      </div>
                    </div>
                    ...
                    <div className="gw-hidden-fields" style={{ display: 'none' }}>
                      <div className="gw-form-group">
                        ...
                        <input id="imap-host" className="gw-input" type="text" defaultValue="imap.gmail.com" />
                      </div>
                      <div className="gw-form-group">
                        <label className="gw-label">포트</label>
                        <input id="imap-port" className="gw-input" type="number" defaultValue="993" />
                      </div>
                      <div className="gw-form-group">
                    ...
                    <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <button type="button" className="gw-preset-chip" id="list-folders-btn" style={{ cursor: 'pointer' }}>
                        <i className="bi bi-arrow-clockwise"></i> 폴더 불러오기
                      </button>
                      <span id="folder-list-hint" style={{ fontSize: '1rem', color: '#73879c' }}></span>
                    </div>
                  </div>
                  ...
                  <div className="gw-card">
                    ...
                    <div className="gw-form-group" style={{ marginBottom: '18px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                        ...
                        <button type="button" className="gw-select-all-btn" id="select-all-btn" style={{ display: 'none' }}>
                          전체 선택
                        </button>
                      </div>
                      <div className="gw-folder-list" id="folder-list">
                        <span className="gw-folder-empty">"폴더 불러오기"를 눌러 수집할 폴더를 선택하세요.</span>
                      </div>
                    </div>
              ...
              <div className="gw-split-left-content">
                <div className="gw-card">
                  ...
                  <label className="gw-dropzone" id="message-dropzone" htmlFor="message-file-input">
                    ...
                    <div className="gw-dropzone-text">txt 파일을 여기로 끌어다 놓거나 클릭해서 선택하세요.</div>
                    <div className="gw-dropzone-sub">카카오톡은 채팅방에서 대화 내보내기가 가능합니다.</div>
                    <div className="gw-dropzone-file" id="message-file-name" style={{ display: 'none' }}></div>
                  </label>
                  <input type="file" id="message-file-input" accept=".txt" style={{ display: 'none' }} />

                  <div className="gw-form-grid" style={{ marginTop: '18px' }}>
                    <div className="gw-form-group">
                      <label className="gw-label">대화방 이름</label>
                      <input id="message-room-name" className="gw-input" type="text" placeholder="예: 가족톡" />
                    </div>
                    <div className="gw-form-group">
                      <label className="gw-label">모드</label>
                ...
                >
                  ...
                  <span id="message-upload-hint" style={{ fontSize: '1rem', color: '#73879c' }}></span>
                </div>
              </div>
