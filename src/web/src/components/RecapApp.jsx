...
/**
"Recap" 페이지(recap.html) 전체를 감싸는 최상위 React 컴포넌트 — 히어로, 발신자/수신자·키워드·친밀도· 동기화 통계 카드, 사이드바(공용 vanilla 모듈)까지 한 번에 마운트한다.
사이드바 선택이 바뀌면 다섯 개 통계 API를 병렬로 다시 불러오되, 그 사이 또 다른 선택으로 바뀌면 늦게 도착한 결과는 화면에 반영하지 않는다(React 이펙트의 표준 ignore-flag 패턴 — 기존의 수동 generation 카운터와 동일한 효과).

Top-level React component wrapping the entire Recap page (recap.html) — mounts the hero, the sender/keyword/affinity/sync stat cards, and the shared sidebar (vanilla module) in one place.
When the sidebar selection changes, the five stat APIs are re-fetched in parallel; if the selection changes again before they resolve, the stale results are ignored (React's standard effect ignore-flag pattern — equivalent to the original's manual generation counter).
 */
...
function loadingCardStates() {
  return {
    sender: LOADING_STATE,
    mysent: LOADING_STATE,
    keyword: LOADING_STATE,
    affinity: LOADING_STATE,
    sync: LOADING_STATE,
  };
}
...
function RecapApp() {
  ...
  // 사이드바 + 계정 선택 상태 동기화는 공용 vanilla 모듈(filterSync.js/appSidebar.js)이 전담 — #app-sidebar placeholder에 마운트만 해주고, 선택이 바뀔 때마다 넘어오는 콜백을 state로 받는다.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      // 다섯 API를 기다리는 동안 다른 계정이 또 선택됐으면(=ignore) 이 결과는 이미 화면에 안 맞는 낡은 데이터다.
      // 캐시에는 넣어 두되(나중에 이 계정으로 돌아왔을 때 재사용) 화면에는 반영하지 않는다.
  ...
  const heroSub =
    name !== "-" ? `${name}님의 전체적인 통계를 보여줍니다` : "내 메일함 통계 한눈에 보기";
